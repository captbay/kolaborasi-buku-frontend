...
const useGetCookie = () => {
  ...
  const { token, role, nama_lengkap, id, token_type } = cookieData
  ...
  useEffect(() => {
    setCookie({ token, role, nama_lengkap, id, token_type });
  }, [token, role, nama_lengkap, id, token_type, cookieData, setCookie]);
